...
model <- lm(Data_Scale$TotalScale ~ data_new$winRate) 
#Summarize the Model 
summary(model)


#This Generates the plot of the Linear regression model 
plot( data_new$winRate, Data_Scale$TotalScale, main = "Linear Regression Model", xlab = "Win Rate", ylab = " Scaled Values", pch = 19, col = "blue")

#Creates the regression line for the model 
abline(model,col = "red", lwd =2)
#Plots the model 
plot(Data_Scale$TotalScale ~ data_new$winRate)
#Generates a histogram of the model 
hist(Data_Scale$TotalScale, breaks = 30)
#Summary of skill 
...
Avg_Cluster_Model <- fviz_cluster(Skill_Brack_Avg, data = Data_Scale[,c(14,19)],
...
#Does a K-Means model => Breaks the data into Skilled and Unskilled (??)
 Skill_Brack_Sum<- kmeans(Data_Scale[,c(14,19)], 3, iter.max = 10, nstart = 1)
 print(Skill_Brack_Sum)

#Plots the K-Means Model for SUM
 Sum_Cluster_Model <- fviz_cluster(Skill_Brack_Sum, data = Data_Scale[,c(14,19)],
             palette = c("#00AFBB", "#E7B800","#8c00f0"),
             geom = "point",
             ellipse.type = "convex", 
           ggtheme = theme_bw()
...
Data_Scale_Sum$clusterValue_Avg <- as.factor(Skill_Brack_Sum$cluster)
